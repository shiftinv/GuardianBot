...
class BaseCog(Generic[_TState], commands.Cog):
    ...
    def _read_state(self) -> None:
        if self.__state_path.exists():
            with self.__state_path.open('r') as f:
                self.state = self.__state_type(**json.load(f, object_hook=_set_decoder))
        else:
    ...
    def _write_state(self) -> None:
        ...
        with self.__state_path.open('w') as f:
            json.dump(asdict(self.state), f, cls=_SetEncoder, indent=4)
